...
import { getClimateArgs, onClimateArgs, isEmbedded } from '../climateclock/bridge.js';
import { ReportSync } from '../climateclock/ReportSync.js';
import { parseEpw } from '../weather/epw.js';
import { applyFilter } from '../weather/bins.js';
...
import { starterSystems } from '../climateclock/starterSystems.js';
...
export function App(): React.JSX.Element {
  ...
  const [units, setUnits] = useState<UnitSystem>(isEmbedded() ? 'SI' : 'IP');
  ...
  const [comfort, setComfort] = useState<ComfortSettingsState>(() => defaultComfortSettings(isEmbedded() ? 'SI' : 'IP'));
   ...
   */
  const [systems, setSystems] = useState<SessionSystem[]>(() => starterSystems(isEmbedded() ? 'SI' : 'IP'));
  ...
  const filteredWeather = useMemo(() => systems.map(system => applyFilter(weatherFile?.hours ?? NO_HOURS,
    { months: system.weather.months ?? [], hours: system.weather.hours ?? [] })), [systems, weatherFile]);
  ...
  const applyProject = useCallback((text: string): void => {
    ...
    setWeatherFile(file => {
      if (!file) return null;
      const sameStation = next.station && (next.station.wmo && file.location.wmo
        ? next.station.wmo === file.location.wmo
        : next.station.city === file.location.city && next.station.country === file.location.country);
      return sameStation ? convertHoursTo(file, next.units) : null;
    });
  ...
  const [climateArgs, setClimateArgs] = useState(getClimateArgs);
  const [climateReady, setClimateReady] = useState<string | null>(null);
  useEffect(() => onClimateArgs(setClimateArgs), []);
  useEffect(() => {
    if (!climateArgs?.station_key || climateReady === climateArgs.station_key) return;
    const key = 'climateclock:studio:' + climateArgs.station_key;
    let restored = false;
    try {
      const stored = window.sessionStorage.getItem(key);
      if (stored) {
        const saved = JSON.parse(stored) as { project: string; weather: EpwFile | null };
        if (readProject(saved.project).project) {
          applyProject(saved.project);
          setWeatherFile(saved.weather);
          restored = true;
        }
      }
      // A new ClimateClock session starts fresh without retaining previous locations.
      for (const old of Object.keys(window.sessionStorage)) {
        if (old.startsWith('climateclock:studio:') && old !== key) window.sessionStorage.removeItem(old);
      }
    } catch { /* The studio also works when browser storage is unavailable. */ }
    if (!restored) {
      const file = parseEpw(climateArgs.epw_text, 'SI');
      setUnits('SI'); setComfort(defaultComfortSettings('SI'));
      setSystems(starterSystems('SI').map(system => ({ ...system, weather: { ...system.weather, mode: 'density' } })));
      setActiveSystem(0); setWeatherFile(file);
      setPressureMode('explicit'); setExplicitPressure(String(climateArgs.pressure_kpa));
      setMeta({ name: climateArgs.station_label });
    }
    setLoadProblems([]);
    setClimateReady(climateArgs.station_key);
  }, [climateArgs, climateReady, applyProject]);
  useEffect(() => {
    if (!climateReady) return;
    const timer = setTimeout(() => {
      try { window.sessionStorage.setItem('climateclock:studio:' + climateReady, JSON.stringify({ project: writeProject(toProject(session)), weather: weatherFile })); }
      catch { /* Exporting a project remains available if the browser quota is full. */ }
    }, 120);
    return () => clearTimeout(timer);
  }, [session, weatherFile, climateReady]);
  ...
  function renderFace(index: number): React.JSX.Element {
    ...
    return (
      ...
      >
        <WeatherLayer
          hours={filteredWeather[index] ?? NO_HOURS}
  ...
  return (
    ...
    <div className={`app${focusChart ? ' chart-focused' : ''}`}>
      <ReportSync stationKey={climateReady} session={session} weatherFile={weatherFile} width={size.width} height={size.height} atmosphere={atmosphere} units={units} zones={zones}
        cases={systems.map((system, index) => ({ label: systemLabel(system, index), chartRef: faceRef(index), domain: system.domain,
          weather: { hours: filteredWeather[index] ?? NO_HOURS, mode: system.weather.mode ?? 'off' }, solved: solvedSystems[index]!.airstreams[0]! }))} />
      <header className="app-header">
        {!isEmbedded() && <div className="brand">
        ...
        <div className="header-toggles">
          <div className="unit-toggle">
            <button type="button" aria-pressed={focusChart} onClick={() => setFocusChart(value => !value)}>{focusChart ? 'Show controls' : 'Focus chart'}</button>
          </div>
        ...
        <aside className="panel panel-right">
          ...
          <Collapsible title="Save, share, export" defaultOpen={false}>
            <ExportPanel
              ...
              weather={{ hours: filteredWeather[activeSystem] ?? NO_HOURS, mode: weather.mode }}
          ...
          <p className="comfort-note">Adapted from <a href="https://github.com/patpease/psychrometric-studio" target="_blank" rel="noreferrer">Pease Studio’s Psychrometric Studio</a> · <a href="./upstream-license.txt" target="_blank" rel="noreferrer">MIT license</a>.</p>
