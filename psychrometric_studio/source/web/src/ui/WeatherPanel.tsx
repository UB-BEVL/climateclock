import { isEmbedded, callClimate, base64Blob } from '../climateclock/bridge.js';
/**
...
export function WeatherPanel({
  ...
  const loadFromUrl = async (): Promise<void> => {
    ...
    try {
      if (isEmbedded()) {
        const result = await callClimate('weather', { url: url.trim() });
        await load(new File([base64Blob(result.data, 'application/zip')], archiveNameFrom(url), { type: 'application/zip' }));
        setUrl(''); return;
      }
