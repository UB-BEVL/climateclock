...
function prepareChart(
  ...
  const outputHeight = height + (withFooter ? 38 : 0);
  ...
  clone.setAttribute('viewBox', `0 0 ${width} ${outputHeight}`);
  ...
  background.setAttribute('height', String(outputHeight));
  ...
  if (withFooter) clone.appendChild(footer(options, width, outputHeight));
  return { clone, width, height: outputHeight };
}
